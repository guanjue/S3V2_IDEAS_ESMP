	...
	if normalization=='T':
		...
		for mk in mks:
			...
			if uniq_mk_num==1:
				...
				### S3V2 normalization
				b=call('python '+script_dir+'/s3v2norm.pipemultithreads_IDEAS.py'+' -n '+str(threads)+' -e '+str(local_bg_bin)+' -t '+mk[0]+'.file_list.S3V2.txt'+' -k '+mk[0]+'_commonpkfdr01_z.cpk.txt'+' -g '+mk[0]+'_commonpkfdr01_z.cbg.txt'+' -s '+script_dir+' -i '+mk[0]+'_commonpkfdr01_z.allpk.txt'+' -l '+str(0.0001), shell=True)
			### get S3V2 ave
