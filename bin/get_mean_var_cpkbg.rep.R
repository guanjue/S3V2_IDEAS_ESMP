...
get_pk_bg_non0mean_non0var_mse_r2 = function(f1_file,f2_file, cpk_used, cbg_used){
	...
	sm = 1
	r2cpk = max(r2(log2(f1cpk+sm),log2(f2cpk+sm)),r2(log2(f2cpk+sm),log2(f1cpk+sm)))
	### cbg
	f1cbg = f1[cbg_used]
	f2cbg = f2[cbg_used]
	f1cbgnon0_log2 = log2(f1cbg[f1cbg!=0])
	f2cbgnon0_log2 = log2(f2cbg[f2cbg!=0])
	f1cbgnon0 = (f1cbg[f1cbg>0])
	f2cbgnon0 = (f2cbg[f2cbg>0])
	r2cbg = max(r2(log2(f1cbg+sm),log2(f2cbg+sm)),r2(log2(f2cbg+sm),log2(f1cbg+sm)))
	### MSE
#	cpk_tmp = c(mean(f1cpknon0),mean(f2cpknon0), sd(f1cpknon0),sd(f2cpknon0), mse(log2(f1cpk+1),log2(f2cpk+1)))
#	cpk_tmp = c(mean(f1cpknon0_log2),mean(f2cpknon0_log2), sd(f1cpknon0_log2),sd(f2cpknon0_log2), mse(log2(f1cpk+sm),log2(f2cpk+sm)))
	cpk_tmp = c(log2(mean(f1cpknon0)),log2(mean(f2cpknon0)), log2(sd(f1cpknon0)),log2(sd(f2cpknon0)), log2(mse(f1cpk,f2cpk)) )

#	cbg_tmp = c(mean(f1cbgnon0),mean(f2cbgnon0), sd(f1cbgnon0),sd(f2cbgnon0), mse(log2(f1cbg+1),log2(f2cbg+1)))
#	cbg_tmp = c(mean(f1cbgnon0_log2),mean(f2cbgnon0_log2), sd(f1cbgnon0_log2), sd(f2cbgnon0_log2), mse(log2(f1cbg+sm),log2(f2cbg+sm)))
	cbg_tmp = c(log2(mean(f1cbgnon0)),log2(mean(f2cbgnon0)), log2(sd(f1cbgnon0)), log2(sd(f2cbgnon0)), log2(mse(f1cbg,f2cbg)) )

#	non0mean_non0var_mse_tmp = c( cpk_tmp, cbg_tmp, mse(log2(f1+sm),log2(f2+sm)), r2cpk, r2cbg, max(r2(log2(f1+sm),log2(f2+sm)),r2(log2(f2+sm),log2(f1+sm))) )
	non0mean_non0var_mse_tmp = c( cpk_tmp, cbg_tmp, log2(mse(f1,f2)), r2cpk, r2cbg, max(r2(log2(f1+sm),log2(f2+sm)),r2(log2(f2+sm),log2(f1+sm))) )
	print(cpk_tmp)
	print(cbg_tmp)
	rm(f1)
	rm(f2)
