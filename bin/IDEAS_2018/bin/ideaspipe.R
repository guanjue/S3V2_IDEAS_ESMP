...
#----------------FUNCTION DEFINITION----------------------------------
runideas<-function(args, out, head, inv=NULL, wait=TRUE)
{	str=paste("ideas", paste(args, collapse=" "), "-thread", threadn, "-o", out);
	...
	rt=system(paste("sh ", out, ".script", sep=""), wait=wait);
	system(paste("rm ", out, ".script", sep=""));
	if(rt!=0) stop("IDEAS quit unexpectedly");
}
