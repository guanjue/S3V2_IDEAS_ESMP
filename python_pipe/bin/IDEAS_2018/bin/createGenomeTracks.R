...
createTrack<-function(statefiles, genomefile, outpref, statecolor, header, statename=NULL, logfile=NULL)
{	message("Reading state file: ", appendLF=FALSE);
	library("data.table");
	genomesz = read.table(genomefile);  #expects bed 3
	g=NULL;
	for(i in statefiles)
	{	message(paste(i," ",sep=""),appendLF=F);
		...
		t=NULL;
		#trim anything past end of chrom
		for(j in 1:dim(genomesz)[1])
		{	t=c(t,which(tg[,2]==as.character(genomesz[j,1]) & as.numeric(tg[,4])>as.numeric(genomesz[j,3])));
		}	
        ...
        if (length(logfile) > 0) { write("Done reading statefile", file=logfile, append=TRUE) }
	message("Done");
	...
	{	tstate = state[,i];
		...
		message(paste("make track", i), sep=" ");
		x = cbind(np[,1:3],statename[as.integer(np[,4])+1],1000,".",np[,2:3],statecolor[as.numeric(np[,4])+1]);
		if (length(logfile) > 0) { write(paste("Writing bed file ", outpref,".",i,".bed1",sep=""), file=logfile, append=TRUE) }
		write.table(as.matrix(x),paste(outpref,i,"bed1",sep="."),quote=F,row.names=F,col.names=F);
		#chroms are sorted in genome file, windows should be in order
		message("Convert to bb");
		ret=system(paste("bedToBigBed ", outpref, ".", i, ".bed1 ", genomefile, " ", outpref, ".",i,".bb",sep=""));
                if (ret == 0) {
		   if (length(logfile) > 0) { write("Converted to bigBed", file=logfile, append=TRUE) }
		   system(paste("rm ", paste(outpref, i,"bed1",sep=".")));
                } else {
		   if (length(logfile) > 0) { write("Failed to convert bed to bigBed", file=logfile, append=TRUE); }
                }
	}
	return(cells);
}
...
run<-function(statefiles, hubid, genomeid, genomefile, statecolor, targetURL="http://bx.psu.edu/~yuzhang/tmp/", trackfolder=NULL, hubname = NULL, cellinfo = NULL, header=NULL,statename=NULL, email=NULL, logfile=NULL)
{
...
#	if(file.exists("sort-bed") == FALSE) { message("Cannot find sort-bed in the folder where you run this function."); return(0); }
	...
	dir.create(trackfolder,showWarnings=FALSE);
        ...
        if (length(logfile) > 0) { write("Creating tracks", file=logfile, append=TRUE) }
	cells=createTrack(statefiles, genomefile, paste(trackfolder, hubid, sep=""), statecolor, header=header,statename=statename, logfile=logfile);
	...
	write.table(c(paste("hub", hubid), paste("shortLabel", hubid), paste("longLabel", hubname), paste("genomesFile genomes_", hubid, ".txt", sep=""), paste("email", email)), paste(trackfolder, "hub_", hubid, ".txt",sep=""), quote=F,row.names=F,col.names=F);

	return(1);
}

createHeatmap<-function(parafile, statecolor = NULL, markcolor = NULL, cols=c("white","dark blue"), show=TRUE,fout=NULL, sortstate=TRUE,scale=FALSE,logfile=NULL)
...
#}
	...
	{	if(length(markcolor)==0)
		{	markcolor=t(col2rgb(terrain.colors(ceiling(p))[1:p]));	
			...
			write("markcolor from hardcoding ", file=logfile, append=TRUE);
		} else { #else order by marks and remove mark
			write("parsing markcolor from user", file=logfile, append=TRUE);
			markcolor=markcolor[order(marks),];
                        write("have markcolor from user ", file=logfile, append=TRUE); #test print
			markcolor=markcolor[,-1];
			markcolor=matrix(sapply(markcolor, as.numeric), ncol=3);
                }
